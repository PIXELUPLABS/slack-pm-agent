...
- **Config**: `config/conventions.json` + `config/index.js` loader — clients→list mapping, priorities, naming, statuses, Slack↔ClickUp user mapping, roles, drafts channel, update schedule, non-client internal lists (`internal_lists`, e.g. Automation Ideas). Single source of truth, loaded at startup.
